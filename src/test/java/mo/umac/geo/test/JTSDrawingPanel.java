...
/**
 * Testing the api of JTS package including drawing point on a map etc.
 * 
 * @author kate
 * 
 */
public class JTSDrawingPanel extends JPanel {
	...
	String shpFileName = UScensusData.STATE_SHP_FILE_NAME;
	String dbfFileName = UScensusData.STATE_DBF_FILE_NAME;
	...
	protected void paintComponent(Graphics g) {
		...
		if (!geometries.isEmpty()) {
			...
			Paint polyPaint = new GradientPaint(0, 0, Color.CYAN, 100, 100,
					Color.MAGENTA, true);
	...
	private void setTransform() {
		...
		Rectangle drawingRect = new Rectangle(visRect.x + MARGIN, visRect.y
				+ MARGIN, visRect.width - 2 * MARGIN, visRect.height - 2
				* MARGIN);

		double scale = Math.min(drawingRect.getWidth() / env.getWidth(),
				drawingRect.getHeight() / env.getHeight());
	...
	public static void main(String[] args) throws Exception {
		...
		LineString line = (LineString) reader
				.read("LINESTRING(20 20, 20 25, 25 25, "
						+ "25 15, 15 15, 15 30, 30 30, 30 10, "
						+ "10 10, 10 35, 35 35, 35 5)");
		panel.addGeometry(line);

		line = (LineString) reader
				.read("LINESTRING(-10 40, 5 50, 20 40, 35 50, 50 40)");
		panel.addGeometry(line);

		Polygon poly = (Polygon) reader
				.read("POLYGON((-10 -10, 0 0, 40 0, 50 -10, 40 -20, 0 -20, -10 -10), "
