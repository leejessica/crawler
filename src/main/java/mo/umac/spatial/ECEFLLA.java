...
public class ECEFLLA {
	...
	double[] ret = { radians2Degree(lat), radians2Degree(lon),
		radians2Degree(alt) };
	...
	double radiusSurfacePoint = Math.sqrt(Math.pow(r, 2)
		/ (1 + (1 / (Math.pow((1 - f), 2)) - 1)
			* Math.pow(Math.sin(lamdaSeaLevel), 2)));
	double x = radiusSurfacePoint * Math.cos(lamdaSeaLevel) * Math.cos(lon)
		+ alt * Math.cos(lat) * Math.cos(lon);
	double y = radiusSurfacePoint * Math.cos(lamdaSeaLevel) * Math.sin(lon)
		+ alt * Math.cos(lat) * Math.sin(lon);
	double z = radiusSurfacePoint * Math.sin(lamdaSeaLevel) + alt
		* Math.sin(lat);
	double[] ret = { x, y, z };
	return ret;
    }
